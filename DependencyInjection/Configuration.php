    ...
    public function getConfigTreeBuilder()
    {
        ...
        $rootNode
          ->children()
              ...
              ->end()
              ->scalarNode('request_id_provider')
                  ->defaultValue('Silpion\LoggerExtraBundle\Logger\Provider\Request\UniqRequestIdProvider')
                  ->info('Class used for providing request ids.')
              ->end()
              ->booleanNode('session_id')
                  ->defaultFalse()
                  ->info('If a salted SHA1 of the session_id should be added to the [extra] section of each log message.')
              ->end()
              ->scalarNode('session_id_provider')
                  ->defaultValue('Silpion\LoggerExtraBundle\Logger\Provider\Session\SymfonySessionIdProvider')
                  ->info('Class used for providing session ids.')
              ->end()
              ->booleanNode('session_start')
                  ->defaultFalse()
                  ->info('If the session should be started, so the session_id will always be available.')
              ->end()
              ->booleanNode('process_id')
                ->defaultFalse()
                ->info('If the PID (Process-ID) of the PHP Interpreter should be added to the [extra] section of each log message.')
              ->end()
              ->arrayNode('additions')
